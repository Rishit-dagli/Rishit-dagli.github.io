...
class GradientDescentScene(ThreeDScene):
    def construct(self):
        ...
        gradient_vector = Arrow(start=axes.c2p(1, 1, function(1, 1)), end=axes.c2p(1.5, 1.5, function(1, 1)), buff=0, color=BLUE)
        gradient_label = MathTex("-\\nabla f(x_k)").next_to(gradient_vector.get_end(), UP)

        # Define the momentum vector at the same point as the ball.
        momentum_vector = Arrow(start=axes.c2p(1, 1, function(1, 1)), end=axes.c2p(0.5, 0.5, function(1, 1)), buff=0, color=GREEN)
        ...
        self.add(axes, surface, ball, gradient_vector, gradient_label, momentum_vector, momentum_label)
