...
class FlatSharpMinimaScene(Scene):
    def construct(self):
        ...
        flat_minima_label = MathTex("m_1", color=WHITE).next_to(flat_minima_dot, RIGHT, buff=0.2)
        sharp_minima_label = MathTex("m_2", color=WHITE).next_to(sharp_minima_dot, RIGHT, buff=0.2)

        # Labels for the types of minima, positioned relative to their respective graphs
        flat_label = Text("Flat Minima", color=GREEN).scale(0.7).next_to(flat_curve, UP, buff=1.3).shift(2.75 * LEFT)
        sharp_label = Text("Sharp Minima", color=RED).scale(0.7).next_to(sharp_curve, UP, buff=1.3).shift(RIGHT).shift(RIGHT).shift(RIGHT)
        
        # Group the components of each graph
        flat_graph = VGroup(axes, flat_curve, flat_minima_dot, flat_minima_label)
        sharp_graph = VGroup(axes.copy(), sharp_curve, sharp_minima_dot, sharp_minima_label)

        # Position the graphs closer to each other
        graphs = VGroup(flat_graph, sharp_graph).arrange(RIGHT, buff=1) # reduced buff for tighter layout
