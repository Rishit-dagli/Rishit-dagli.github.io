...
$$\tag{1}\theta_{t+1} = \underbrace{\theta_t}_{\substack{\text{previous}\\\text{params}}} - \overbrace{\eta}^{\text{step-size}} \frac{\partial\overbrace{\hat{L}(\theta_t, x, y)}^{\substack{\text{stochastic estimate}\\\text{of loss}}}}{\partial\underbrace{\theta_t}_{\substack{\text{previous}\\\text{params}}}}$$
